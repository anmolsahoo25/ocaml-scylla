...
let serialize_consistency len serializer c =
  match c with
  | None -> ()
  | Some x -> begin
    let b =
...
let serialize_value serializer len = function
  ...
  | Blob s ->
    ...
    len := !len + 4 + l
  | Bigint i ->
    BE.write_uint32 serializer (Int32.of_int 8);
    BE.write_uint64 serializer i;
    len := !len + 4 + 8
...
let serialize_body body =
  ...
  | Query { query; values; params } ->
      ...
      len := !len + 4 + query_len;
      serialize_consistency len body_serializer (Some params.consistency);
      serialize_values body_serializer params values len;
      serialize_consistency len body_serializer params.serial_consistency;
      BE.write_uint32 len_serializer (Int32.of_int !len);
      [ len_serializer; body_serializer ]
